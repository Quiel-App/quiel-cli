...
**Что менялось между версиями — [CHANGELOG.ru.md](https://github.com/Quiel-App/quiel-cli/blob/main/CHANGELOG.ru.md).**
...
- **Даёт агенту простаивать и будит его, когда появляется работа.** Пустая очередь — нормальное состояние, а не сбой: агент завершает ход, вместо того чтобы просить задачу снова и снова. Фоновый ожидатель замечает, что в очереди появилось что-то для него, и будит сессию. Ожидание стоит сокета и пары байт — не ходов модели и не контекста.
- **Считает, во сколько токенов обошлась задача**, и отправляет итоги — чтобы потом можно было посмотреть на проект целиком.
- **Спасает незаконченную работу, когда кончается лимит подписки.** Коммитит то, что агент успел, и отправляет в ветку задачи с пометкой «прервано» — чтобы следующий человек или агент продолжил с этого места, а не начинал заново. В основную ветку не пишет никогда.
...
**Клиент действительно коммитит и пушит — и только в ветку задачи.** Это случается в одном случае: у вас кончился лимит подписки посреди работы. Потерять полдня работы хуже, чем получить лишнюю ветку в репозитории, — но знать об этом вы должны, и знать, куда именно.
...
У Claude Code на событии `Stop` прописываются **две** команды, и это не дубль: одна решает, можно ли агенту остановиться, вторая ждёт появления работы, пока он стоит, и будит сессию. Вторая объявлена с `asyncRewake` — работает в фоне и ничего не блокирует.
...
**После обновления пакета выполните `quiel init` заново.** `npm i -g` меняет бинарь, но не трогает конфигурацию агента — а новый выпуск может добавить хук. Версия 0.6.0 так добавила ожидатель задач, и агент, обновлённый без повторного `init`, вместо простоя замолкал. Клиент теперь это замечает и продолжает работать по-старому, но включает новое поведение именно `init`. Если в этот момент запущен MCP-сервер — переподключите его (`/mcp` в Claude Code): старый процесс отвечает, пока этого не сделать.
...
| `quiel wait-for-task` | Ждёт появления задачи и будит спящую сессию (запускается хуком, не руками) |
...
Здесь — документация и трекер задач для `@quiel/cli`. Quiel — коммерческий продукт: платформа работает на наших серверах, её исходники не публикуются. Клиент — та часть, которая работает у вас на машине, и он выходит под разрешительной лицензией, чтобы установка не требовала согласования с юристами. Что клиент делает с вашей машиной, описано выше, и каждый файл, который он пишет, лежит в вашем же каталоге проекта — его можно прочитать.

## Лицензия

Клиент — этот пакет — распространяется под лицензией Apache-2.0, полный текст в файле `LICENSE`. Это про клиент командной строки и только про него: сама платформа Quiel — размещаемый сервис, а не опубликованный код.
